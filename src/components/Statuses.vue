<template>
  <template v-if="!openedStatusModal && !openedEmojiModal">
    <draggable v-if="statuses && statuses.length" v-model="statuses" @change="saveToStorage">
      <template #item="{ element, index }">
        <div class="simple-status_button" @click="setStatus">
          <span @click="openEmoji(element, $event)">{{ element.char }}</span
          >&nbsp;&nbsp;{{ element.name }}

          <!-- Options -->
          <div class="simple-status_button-opt">
            {{ OPTION_BTN_ICON }}
            <div class="simple-status_button-opts">
              <div class="color-red simple-status_button-opts-remove" @click="removeStatus(index, element, $event)">
                {{ REMOVE_BTN_ICON }}
              </div>
              <div @click="editStatus(element, $event)">
                {{ EDIT_BTN_ICON }}
              </div>
            </div>
          </div>
    ...
    <div class="simple-status_button simple-status_button-new" @click="openedStatusModal = true">+ Add New</div>
    <div class="simple-status_button simple-status_button-clear" @click="clearStatus">Clear status</div>
  </template>

  <!-- Form for create or update status-->
  <StatusForm v-if="openedStatusModal" @on-close="closeStatusModal" @on-save="onSaveStatus" :status="selectedStatus" />

  <!-- Emoji list -->
  <Emoji v-if="openedEmojiModal" @on-close="closeEmoji" @on-select="selecteEmoji" />
</template>
...
import { STATUSES_LIST_KEY, OPTION_BTN_ICON, REMOVE_BTN_ICON, EDIT_BTN_ICON } from '../config';
